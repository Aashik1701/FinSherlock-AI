export default function RiskGauge({ score = 0, riskLevel = 'low', uid = '' }) {
  ...
  return (
    <div className="flex flex-col items-center" style={{ backgroundColor: 'var(--bg-card-hover)', borderRadius: '12px', padding: '8px' }}>
      <svg viewBox="0 0 160 100" className="w-full">
        ...
        <text x={cx} y={cy + 14} textAnchor="middle" fontSize="9" fill="var(--text-muted)" fontFamily="monospace">
          / 100
        </text>
      </svg>
      <span className={`text-[10px] font-bold uppercase tracking-wider mt-1 ${
        riskLevel === 'high' ? 'text-[var(--red)]' : riskLevel === 'medium' ? 'text-[var(--amber)]' : 'text-[var(--emerald)]'
      }`}>
        {riskLevel} risk
      </span>
    </div>
