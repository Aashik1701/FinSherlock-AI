...
graph TB
    ...
    C --> D["⚙️ Brain 2: Deterministic Tool Engine"]
    
    D --> E["📊 EDA Tool"]
    D --> F["🔧 Feature Engineering"]
    D --> G["🔍 Anomaly Detection<br/>(IsolationForest)"]
    D --> H["🕸️ Graph Analysis<br/>(NetworkX)"]
    ...
    N --> O["🖥️ React Dashboard<br/>Investigate · Watchlist · Cases · Live Stream"]
...
### 1. 💵 Structuring Detector (31 U.S.C. § 5324)
...
Compares an account's recent 7-day transaction frequency against its 90-day historical baseline to catch sudden 3×+ behavioral surges.
...
### ⚙️ Two-Brain Execution Plan Trace
...
- **Regulatory Narrative** — grounded explanation citing FinCEN regulations (31 U.S.C. § 5324)
...
### 📈 Visual Analytics
- **24-Hour Velocity Heatmap** — hourly × daily transaction density matrix highlighting off-hours botnet activity
...
> **All fraud-detection decisions are made by deterministic code.** The LLM's only role is to select which tools to invoke for a given query and to power the compliance copilot for informational Q&A.

---

## 🔮 Future Scope

- **Graph Neural Networks (GNN)** for richer community embeddings and link prediction
- **Real-time Kafka/Flink streaming** for production-grade transaction ingestion
- **Multi-jurisdiction regulatory templates** (EU AMLD6, UK MLR, Singapore MAS)
- **Role-based access control (RBAC)** for analyst vs. compliance officer vs. auditor
- **PDF SAR export** with digital signature and tamper-evident hash chain
- **Federated learning** across banking institutions without sharing raw transaction data
